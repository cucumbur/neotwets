def respond_new_replies
  ...
  mentions.each do |tweet|
    ...
    if tweet.in_reply_to_user_id == @twitter_id
      ...
      reply_info = [tweet, user, tokens, command]
      case command
        when 'twegg' # lets a new user get an egg to incubate
          command_twegg reply_info
        when 'egg'
          command_twegg reply_info
        when 'incubate'
          command_incubate reply_info
        when 'reject'
          command_reject reply_info
        when 'name'
          command_name reply_info
        when 'feed'
          command_feed reply_info
        when 'status' # gives user information about current state of twet
          command_status reply_info
        when 'relationship' #gives user information about the relationship between them and their twet
          command_relationship reply_info
        when 'wallet'
          command_wallet reply_info
        # gambling features
        when 'dice' # @neotwetsdev dice bet (neocoins) on (1-6)
          command_dice reply_info
        when 'coinflip' # @neotwetsdev coinflip bet (neocoins) on (heads,tails)
          command_coinflip reply_info
        when 'allowance'
          command_allowance reply_info
        when 'donate' # donate <# coin> to @<username>
          command_donate reply_info
        else
...
def command_twegg reply_info
  tweet, user, tokens, command = reply_info
  if (@tweggs[user]) || @twets[user]
...
def command_incubate reply_info
  tweet, user, tokens, command = reply_info
  if @tweggs[user]
...
def command_relationship reply_info
  tweet, user, tokens, command = reply_info
  if twet = @twets[user]
...
def command_coinflip reply_info
  tweet, user, tokens, command = reply_info
  if (twuser = @users[user]) && tokens.size >= 4
...
def command_allowance reply_info
  tweet, user, tokens, command = reply_info
  if twuser = @users[user]
